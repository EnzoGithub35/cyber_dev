...
<h4 class="center grey-text">
    <?php if (!empty($search)): ?>
        Search Results for "<?php echo htmlspecialchars($search, ENT_QUOTES, 'UTF-8'); ?>"
    <?php else: ?>
                ...
                <div class="col s6 md3">
                    <div class="card z-depth-0">
                        ...
                        <div class="card-content center">
                            <h6><?php echo htmlspecialchars($story['title'], ENT_QUOTES, 'UTF-8') ?></h6>
                            <ul>
                                <?php foreach (explode(',', $story['content']) as $ing): ?>
                                    <li><?php echo htmlspecialchars($ing, ENT_QUOTES, 'UTF-8') ?></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                        <div class="card-action right-align">
                            <a href="details.php?id=<?php echo htmlspecialchars($story['id'], ENT_QUOTES, 'UTF-8') ?>" class="brand-text">MORE INFO</a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php elseif (!empty($search) && count($stories) === 0): ?>
            <p class="center grey-text">No matching search results for "<?php echo htmlspecialchars($search, ENT_QUOTES, 'UTF-8'); ?>"</p>
        <?php endif; ?>
    </div>
</div>
...
<script>
    ...
    var searchTerm = "<?php echo htmlspecialchars($search, ENT_QUOTES, 'UTF-8'); ?>";
</script>
